...
func (c *core) handleRoundChange(msg *message, src istanbul.Validator) error {
	...
	logger.Debug(fmt.Sprintf("=====> Handling round change msg, total number of round change msg for the round %v, sequence %v, is %v, waiting for round change? %v, current F() is %v", roundView.Round, roundView.Sequence, num, c.waitingForRoundChange, c.valSet.F()))
	// Once we received f+1 ROUND CHANGE messages, those messages form a weak certificate.
	// If our round number is smaller than the certificate's round number, we would
	// try to catch up the round number.
	if c.waitingForRoundChange && num == int(c.valSet.F()+1) {
		if cv.Round.Cmp(roundView.Round) < 0 {
			logger.Info(fmt.Sprintf("====>Initiate Round change after f+1 messages: current round - %v, seq - %v", c.current.Round(), c.current.Sequence()))
